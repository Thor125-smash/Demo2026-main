...
## 1. Настройка RADIUS-сервера (srvs1-cod)
...
## 2. Настройка RADIUS на cod (EcoRouter)

### 2.1 Подключение к RADIUS-серверу

```
cod(config)#security none 
cod(config)#aaa radius-server 192.168.10.1 port 1812 secret P@ssw0rd auth
cod(config)#
```
...
```
cod(config)#aaa precedence local radius
...
## 3. Настройка PAM RADIUS на switch1-cod и switch2-cod (ALT Linux)
...
Проверить доступ по SSH из-под пользователя `netuser` с паролем `P@ssw0rd` (например с srvs1-cod):
