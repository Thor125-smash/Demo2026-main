...
## 1. Подготовка dcserv-a (ALT Server)
...
```bash
host -t SRV _kerberos._udp.oDomain.
```
...
### 4.1 Установка пакетов (client1-a и client2-a)
...
- Домен: `oDomain`
- Имя компьютера: `client1-a` (или `client2-a`)
...
ПКМ на `oDomain` → **Создать подразделение**
...
### 8.1 Вход под ofadmin1 (client2-a)

![Вход ofadmin1](../images/ad_login_ofadmin1.png)

### 8.2 Вход под ofuser1 (client2-a)
...
### 9.1 Установка gpupdate (client1-a и client2-a)
...
В ADMC: **Объекты групповой политики** → ПКМ на `oDomain` → **Создать политику и связать с этим подразделением**
...
Перезагрузить client1-a и client2-a для применения политик.
